"""Backtest MLB win-probability Elo model.

Train (hyperparameter fitting only): 2019-2025 -- seven seasons including the
60-game COVID 2020 season. Held-out test: every 2026 game already completed
(the current season, entirely unseen during fitting). Ratings carry forward
chronologically through the full history into the test season, exactly as
they do in live use.
"""
import sys
import pathlib
...
def main():
    ...
    print(f"Train: {len(train_df)} games ({TRAIN_SEASONS[0]}-{TRAIN_SEASONS[-1]}). "
          f"Test: {test_mask.sum()} completed {TEST_SEASONS[0]} games")
    ...
    print(f"\n=== Out-of-sample results, {TEST_SEASONS} completed games ===")
